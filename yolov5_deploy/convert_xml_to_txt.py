...
def convert_xml_to_yolo(xml_file, output_dir, classes):
    ...
    yolo_data = []
    wnit_to_num_dict = {y.split()[0]: y.split()[1] for y in classes}
    # Extract object details
    for obj in root.findall('object'):
        class_name = obj.find('name').text
        if class_name in wnit_to_num_dict:
            class_id = wnit_to_num_dict[class_name]
...
def iterate_and_process_xml_files(directory):
    # xml_file = r"C:\Users\eitan\OneDrive\Desktop\Master\study\B.A\Year_4\final_project\n01440764\n01440764_18.xml"
    output_dir = r"C:\Users\eitan\OneDrive\Desktop\Master\study\B.A\Year_4\final_project"
    classes_file = r"C:\Users\eitan\OneDrive\Desktop\Master\study\B.A\Year_4\final_project\Engineering-Project\yolov5_deploy\wnid_to_num.txt"  # Path to your classes.txt file
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith('.xml'):
                ...
                out_path = os.path.dirname(xml_file.replace("Annotations_xml", "Annotations"))
                convert_xml_to_yolo(xml_file, out_path, classes)


# Define the mapping from human-readable names to numerical numbers
name_to_number = {
    'tench, Tinca tinca': 0,
...
# Function to load the WNID to human names file
def load_wnid_to_names(file_path):
    wnid_to_names = {}
    with open(file_path, 'r') as file:
        for line in file:
            ...
            wnid = parts[0]
            names = " ".join(parts[1:])
            wnid_to_names[wnid] = names
    return wnid_to_names


# Function to create the WNID to numerical mapping file
def create_wnid_to_number_mapping(wnid_to_names, name_to_number, output_file):
    with open(output_file, 'w') as file:
        for wnid, names in wnid_to_names.items():
            # Split the names by comma and match to the first part (primary name)
            primary_name = names
            if primary_name in name_to_number:
                number = name_to_number[primary_name]
                file.write(f"{wnid} {number}\n")


if __name__ == "__main__":

    path = r"C:\Users\eitan\OneDrive\Desktop\Master\study\B.A\Year_4\final_project\datasets\imagenet\ILSVRC\Annotations_xml\CLS-LOC"
    iterate_and_process_xml_files(path)


    # wnid_to_names_file = 'wnid_to_name.txt'
    # output_mapping_file = 'wnid_to_num.txt'
    #
    # # Load the WNID to human names mapping
    # wnid_to_names = load_wnid_to_names(wnid_to_names_file)
    #
    # # Create the WNID to numerical mapping file
    # create_wnid_to_number_mapping(wnid_to_names, name_to_number, output_mapping_file)
